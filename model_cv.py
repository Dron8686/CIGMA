...
try:
    ...
    print("Warning: parameters.py not found.")
...
class GraphHSA(BaseModel):
    ...
    def forward(self, symptoms: torch.LongTensor, drug: torch.LongTensor,
        ...
        symptom_agg_embeddings = self._aggregation(symptom_i_list)
        drug_agg_embeddings = self._aggregation(drug_i_list)
        ...
        drug_neighbor_emb_hop1 = drug_neighbors_emb_list[0] if self.n_hop > 0 else None
        symptom_neighbor_emb_hop1 = symptom_neighbors_emb_list[0] if self.n_hop > 0 else None
        ...
        return score, combined_emb, loss_hybrid, score_pro, score_gcn
    ...
    def _aggregation(self, item_i_list):
        if not item_i_list:
            if self.n_hop == 0:
                return torch.zeros((1, self.emb_dim * self.n_hop), device=self.aggregation_function.weight.device)  #

        item_i_concat = torch.cat(item_i_list, 1)
        expected_dim = self.emb_dim * self.n_hop

        if item_i_concat.shape[1] == 0 and expected_dim == 0:
            batch_size = item_i_list[0].shape[0] if item_i_list else 1  # 猜测 batch size
            return torch.zeros((batch_size, self.emb_dim), device=self.aggregation_function.weight.device)

        if item_i_concat.shape[1] != expected_dim:
            # print(f"Warning: Dimension mismatch in _aggregation. Expected {expected_dim}, got {item_i_concat.shape[1]}. Check n_hop consistency.")
            if item_i_concat.shape[1] > expected_dim:
