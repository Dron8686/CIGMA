...
* `main_cv.py` and `main_indep.py`: used for cross-validation and independent test training.
* `finetune_cv.py` and `finetune_indep.py`: executed for the fine-tuning stage.
* `model_cv.py` and `model_indep.py`: containing the model architecture definitions.
* `parameters.py`: Configuration file for setting model and training hyperparameters.
* `save_train_val_test_data.py`: used for generating and saving the dataset splits.
* `processed data/`: storing the processed data loaders and edge indices. 
